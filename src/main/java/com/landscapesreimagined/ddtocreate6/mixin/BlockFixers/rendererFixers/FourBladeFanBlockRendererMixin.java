...
@Mixin(value = FourBladeFanBlockRenderer.class, remap = false)
public abstract class FourBladeFanBlockRendererMixin extends KineticBlockEntityRenderer<FourBladeFanBlockEntity> {

    @Shadow protected abstract void renderFlywheel(FourBladeFanBlockEntity be, PoseStack ms, int light, BlockState blockState, float angle, VertexConsumer vb);

    public FourBladeFanBlockRendererMixin(BlockEntityRendererProvider.Context context) {
        super(context);
    }
    ...
    @OnlyIn(Dist.CLIENT)
    @Overwrite
    protected void renderSafe(FourBladeFanBlockEntity be, float partialTicks, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
